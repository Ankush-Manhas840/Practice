...
- Pairing rule: `while low <= high` goes with `high = mid - 1`; `while low < high` goes with `high = mid`. Mixing them (`low <= high` together with `high = mid`) loops forever once `low == high`. The first version of [matrix-median.py](../solutions/binary-search/matrix-median.py) did exactly this and never finished, even on the standard example.
...
| [Matrix median](../solutions/binary-search/matrix-median.py) | smallest first-column value .. largest last-column value | count of elements <= x is >= `(rows*cols + 1) // 2` |
...
- Matrix median (rows sorted, odd total): search over VALUES, not positions. For a candidate `mid`, count the elements `<= mid` by binary searching each row (`upper_bound`, which is where `l` ends up), then look for the smallest value whose count reaches `(rows*cols + 1) // 2`. That value is the median. O(rows × log cols × log(value range)).
